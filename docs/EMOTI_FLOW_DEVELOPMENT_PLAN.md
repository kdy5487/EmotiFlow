...
- [x] **UI 오버플로우 완전 해결** (2026-01-23)
  - [x] 자유일기 감정 캐릭터 바텀 오버플로우 해결 (반응형 적용)
  - [x] 홈화면 일기 작성 선택지 바텀 오버플로우 해결
  - [x] 레이아웃 오류 수정 (RenderBox was not laid out)
- [x] **일기 목록 및 상세페이지 UI 전면 개편** (2026-01-23)
  - [x] 앱바 스타일 통일 (일기 작성 페이지와 일관성)
  - [x] 캐릭터 이미지 적용 (리스트뷰, 그리드뷰, 상세페이지)
  - [x] 일기 구분선 추가 (시각적 구분 명확화)
  - [x] 다크모드 완벽 지원
- [x] **앱 로고 변경** (2026-01-23)
  - [x] Emoti_logo_big.png로 변경
  - [x] pubspec.yaml 및 emotion_character_map.dart 업데이트
- [x] **일기 목록 페이지 UI 전면 개편** (2026-01-23)
  - [x] 그리드 카드 구조 완전 재구성 (제목-내용-날짜만, 오버플로우 완전 방지)
  - [x] 정렬/보기 설정 버튼을 앱바 아래 왼쪽으로 이동
  - [x] 보기 설정 순서 변경 (일반 보기 왼쪽, 그리드 보기 오른쪽)
  - [x] 삭제 모드 진입 시 뒤로가기로 삭제 모드 해제
  - [x] 일기 꾹누르면 삭제모드 작동
  - [x] 앱바 설정 메뉴 재구성 (삭제 모드만 포함)
  - [x] 삭제 버튼 위치 및 색상 개선
- [x] **일기 상세페이지 UI 개선** (2026-01-23)
  - [x] 감정/수치 색상 개선 (배경색과 구별되게)
  - [x] 작성방식 태그 우측 상단 이동
  - [x] 배경색 그라데이션 적용
  - [x] 감정 정보 통합 (기쁨 8 형태)
- [x] **AI 감정 분석 UI 스타일 개선** (2026-01-23)
  - [x] 우리 스타일로 전면 재디자인 (그라데이션, 테두리, 그림자)
  - [x] 카드 섹션 스타일 통일
- [x] **홈 화면 UI 개선** (2026-01-23)
  - [x] 오늘의 일기 작성하기 버튼 색상 변경 (회색 → 보라색)
  - [x] 앱 시작 시 일기 데이터 자동 로드
...
### 📝 해야 할 것 (우선순위순)
1. **필수 (즉시 착수)**
   - [x] 감정 캐릭터 UI 적용 (assets/images/characters/ 활용) ✅ 2026-01-23 완료
   - [ ] 라우팅 버그 수정 (AI 채팅 일기 → 상세 페이지 이슈)
   - [x] 다크모드 색상 대비 및 접근성 완성 ✅ 2026-01-23 완료
