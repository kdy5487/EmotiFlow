...
## 5. 협업 및 답변 규칙 (Point 4)

앞으로의 모든 코드 수정 및 질문에 대해 다음과 같이 답변합니다.

1.  **설계 우선:** 코드를 작성하기 전, 어떤 계층(Domain/Data/Presentation)을 수정할지, 왜 그런 구조를 선택했는지 설명합니다.
2.  **코드 설명:** 작성된 코드의 핵심 로직과 사용된 패턴(Riverpod, Clean Architecture)을 상세히 설명합니다.
3.  **테스트 코드 포함:** 새로운 기능을 구현할 때 해당 기능의 정상 작동을 보장하는 테스트 코드를 함께 제안합니다.
4.  **실무자 관점:** 포트폴리오 및 배포를 고려하여 예외 처리, 성능 최적화, 보안(API Key 관리 등)에 대해 조언합니다.
