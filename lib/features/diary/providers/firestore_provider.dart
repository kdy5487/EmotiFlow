...
class FirestoreProvider {
  ...
  Future<void> saveDiary(DiaryEntry entry) async {
    try {
      ...
      final model = DiaryModel.fromEntity(entry);
  ...
  Future<DiaryEntry?> getDiary(String entryId) async {
    try {
      final doc = await _firestore.collection('diaries').doc(entryId).get();
  ...
  Future<void> seedDummyDiaries(String userId) async {
    ...
    final free = DiaryEntry(
      id: 'dummy_free_${DateTime.now().millisecondsSinceEpoch}',
      userId: userId,
      title: '봄 산책의 기억',
      content: '따뜻한 햇살 아래 공원을 산책했다. 벤치에 앉아 있자니 바람이 살짝 스쳤고, 마음이 한결 가벼워졌다.',
      emotions: const ['평온', '감사'],
      emotionIntensities: const {'평온': 8, '감사': 7},
      ...
      mediaFiles: [
        ...
        MediaFile(
            id: 'm2',
            url: 'https://picsum.photos/seed/sketch/800/600',
    ...
    // AI 대화형
    final ai = DiaryEntry(
      id: 'dummy_ai_${DateTime.now().millisecondsSinceEpoch}',
      userId: userId,
      title: '면접 준비로 인한 긴장',
      content: '다가오는 면접이 걱정되지만 스스로를 믿고 준비해나가기로 했다. 조금씩 정리하다 보니 설렘도 함께 느껴진다.',
      emotions: const ['걱정', '설렘'],
      emotionIntensities: const {'걱정': 7, '설렘': 6},
      ...
      mediaFiles: [
        ...
        MediaFile(
            id: 'm4',
            url: 'https://picsum.photos/seed/notes/800/600',
            type: MediaType.drawing),
      ],
      chatHistory: [
        ChatMessage(
            id: 'c1',
            content: '면접이 다가오는데 떨리고 걱정돼요.',
            isFromAI: false,
            timestamp: DateTime.now()),
        ChatMessage(
            id: 'c2',
            content: '그 마음 충분히 이해해요. 어떤 부분이 가장 걱정되나요?',
            isFromAI: true,
            timestamp: DateTime.now()),
      ],
      tags: const ['면접', '준비', '목표'],
    ...
    await col.doc(free.id).set(DiaryModel.fromEntity(free).toMap());
    await col.doc(ai.id).set(DiaryModel.fromEntity(ai).toMap());
  }
}
...
/// 일기 일회성 조회 Provider
final diariesFutureProvider =
    FutureProvider.family<QuerySnapshot, String>((ref, userId) async {
  final firestore = ref.read(firestoreProvider);
...
final diaryDetailProvider =
    FutureProvider.family<DiaryEntry?, String>((ref, entryId) async {
  final firestore = ref.read(firestoreProvider);
